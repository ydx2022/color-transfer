...
function renderFrameCentered(ctx: CanvasRenderingContext2D, frame: CellFrame, W: number, H: number): void {
  ctx.fillStyle = "#000";
...
export function SendPage() {
  ...
  useEffect(() => {
    ...
    if (!ctx) return;
    renderFrameCentered(ctx, frame, size.w, size.h);
  }, [frame, size]);
  ...
  const exportAllPngs = async () => {
    ...
    for (let k = 0; k < PATTERNS.length; k++) {
      ...
      const ctx = cv.getContext("2d");
      if (!ctx) continue;
      renderFrameCentered(ctx, frames[k], W, H);
  ...
  return (
    <div className="flex h-full flex-col gap-3">
      ...
      {/* 画布：仅纯图案，无任何叠加 */}
      <div ref={containerRef} className="relative flex-1 overflow-hidden rounded-xl border border-ink-700 bg-black">
